...
BEGIN
  ...
  -- Атомарно меняем статус только если он ещё не done/paid
  ...
  IF v_updated = 0 THEN
    -- Уже было done/paid либо запись не найдена — ничего не начисляем
    RETURN jsonb_build_object('ok', true, 'skipped', 'already_done_or_missing');
  END IF;

  -- Цена по наличию фото у задания
  SELECT (image_url IS NOT NULL AND length(trim(image_url)) > 0)
    INTO v_has_image
  FROM public.tasks
  WHERE id = v_task_id;

  v_price := CASE WHEN COALESCE(v_has_image, false) THEN 30 ELSE 20 END;
